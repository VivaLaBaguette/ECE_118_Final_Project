...
#include "BOARD.h"
#include "HSM.h"
#include "Avoid_Tape_Sub.h"

typedef enum {
    InitPSubState,
    SubFirstState,
} Avoid_TapeSubHSMState_t;
...
static Avoid_TapeSubHSMState_t CurrentState = InitPSubState; // <- change name to match ENUM
...
uint8_t InitAvoid_TapeSubHSM(void)
{
    ...
    returnEvent = RunAvoid_TapeSubHSM(INIT_EVENT);
...
ES_Event RunAvoid_TapeSubHSM(ES_Event ThisEvent)
{
    ...
    Avoid_TapeSubState_t nextState; // <- change type to correct enum
